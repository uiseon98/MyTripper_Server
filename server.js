...
app.post('/signup', async (req, res) => {
  ...
  try {
    ...
    const { data, error } = await supabase.from('users').insert([{ name, user_id, password: hashedPassword, mbti }]);

    if (error) {
      return res.status(400).json({ message: '회원가입 실패', error: error.message });
    }
...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    const { data, error } = await supabase.from('users').select('*').eq('user_id', user_id).single(); // 단일 결과 가져오기

    if (error || !data) {
      return res.status(400).json({ message: '로그인 실패: 아이디가 존재하지 않습니다.' });
    }

    // 입력된 비밀번호와 데이터베이스에 저장된 해싱된 비밀번호 비교
    const passwordMatch = await bcrypt.compare(password, data.password);
    if (!passwordMatch) {
      return res.status(400).json({ message: '로그인 실패: 비밀번호가 일치하지 않습니다.' });
    }
...
app.post('/set-new-password', async (req, res) => {
  ...
  try {
    ...
    if (error) {
      return res.status(400).json({ message: '비밀번호 재설정 실패', error: error.message });
    }
...
app.get('/api/reviews', async (req, res) => {
  try {
    ...
    let query = supabase
      .from('travelplan')
      .select('serial_number, sub_title, content_text, review, plan_mbti, post_day, image_url', {
        count: 'exact',
      })
      .order('serial_number', { ascending: false })
      .range(start, end);
